  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        setStatus({
          loading: false,
          success: res.data.message || 'Check your email for reset link',
          error: '',
        });
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-12">
      ...
      <div className="max-w-md w-full space-y-6 p-8 border rounded-md shadow-2xl">
        <h2 className="text-3xl text-black text-center font-special-gothic">Forgot Password</h2>
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-5">
          ...
          <button
            ...
            className="w-full btn-gradient text-white hover:bg-transparent  hover:text-white border-transparent  hover:scale-95 rounded-full py-2.5 text-lg font-medium flex justify-center items-center gap-2 disabled:opacity-50"
          >
            {status.loading ? <><Loader2 className="animate-spin" size={18} /> Sending...</> : 'Send Reset Link'}
          </button>
        </form>
      </div>
    </div>
